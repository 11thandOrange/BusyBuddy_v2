class BOGOBundle {
  ...
  try {
    ...
    if (data.bundles && data.bundles.length > 0) {
      this.bundleExists = true;
      const bundle = data.bundles[0]; // Get the first bundle
      ...
      this.bundleConfig = {
        ...
        discountType: bundle.discountType || "Percentage",
        showTimer: bundle.widgetAppearance?.isShowCountDownTimer || false,
        multipleProducts: bundle.products,
        // multipleProducts: bundle.type !== "Volume Discount",
        volumeOffer:
          bundle.type === "Volume Discount"
            ? `Buy ${bundle.quantityBreaks?.[0]?.quantity || 1}, get ${bundle.discountValue || 10}% OFF`
      ...
      // Filter out null products and log the results
      this.bundleProducts = this.bundleProducts.filter(p => p !== null);
      console.log("All bundle products loaded:", this.bundleProducts);
    } else {
      this.bundleExists = false;
      console.log("No bundles returned from API");
    }
...
${console.log(
  ...
  this.bundleProducts.map((product) => this.renderBundleProduct(product))
)}
...
renderBundleProduct(product) {
  if (!product) return "";
    console.log("Rendering bundle product:", product);
  const price = parseFloat(product.price);
  const comparePrice = parseFloat(product.compare_at_price || 0);

  // Generate options HTML based on bundle data
  const optionsHtml = product.options.map(option => `
    <div class="product-option">
      <label>${option.name}</label>
      <select class="option-select" data-option-id="${option.componentOptionId}">
  ...
  return `
    <div class="bogo-product-card bundle-product">
      ...
      <div class="product-details">
        ...
        <div class="product-price">
          <span class="current-price">${this.formatPrice(price)}</span>
          ${comparePrice > price ? `<span class="original-price">${this.formatPrice(comparePrice)}</span>` : ''}
        </div>
        <div class="product-options">
          ${optionsHtml}
        </div>
