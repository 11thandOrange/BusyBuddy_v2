...
const MixMatchEditorPreview = () => {
  ...
  // Render Mix & Match widget preview
  const renderMixMatchPreview = () => {
    ...
    return (
      <div style={{
        ...
        {/* Tier Badge */}
        <div style={{ display: 'inline-block', padding: '6px 12px', background: '#5169DD', color: '#fff', borderRadius: '20px', fontSize: '12px', fontWeight: 600, marginBottom: '15px' }}>
          Buy {selectedTier}+ Get {currentDiscount}{discountType === 'Percentage' ? '%' : '$'} Off
        </div>

        {!hasProducts ? (
          <div style={{ padding: '30px 15px', textAlign: 'center', backgroundColor: colorSettings.primaryBackgroundColor, borderRadius: `${Math.max(0, cornerRadius - 5)}px`, border: `1px solid ${colorSettings.borderColor}` }}>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '15px' }}>
              {selectedProducts.slice(0, 4).map((product, index) => (
                <div key={product.productId || index} style={{ padding: '10px', borderRadius: `${Math.max(0, cornerRadius - 5)}px`, backgroundColor: colorSettings.primaryBackgroundColor, border: `1px solid ${colorSettings.borderColor}` }}>
            ...
            {/* Total */}
            <div style={{ padding: '12px', backgroundColor: colorSettings.primaryBackgroundColor, borderRadius: `${Math.max(0, cornerRadius - 5)}px`, border: `1px solid ${colorSettings.borderColor}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                ...
                <div>
                  {(() => {
                    ...
                    return (
                      <>
                        <span style={{ fontWeight: 700, fontSize: '16px', color: colorSettings.primaryTextColor }}>${discountedTotal.toFixed(2)}</span>
                        {discountedTotal < originalTotal && <span style={{ marginLeft: '8px', fontSize: '13px', textDecoration: 'line-through', color: '#999' }}>${originalTotal.toFixed(2)}</span>}
                      </>
                    );
                  })()}
                </div>
