...
const BuyoneGetoneActions = React.forwardRef(({ onSuccess, editData }, ref) => {
  ...
  useEffect(() => {
    if (editData) {
      ...
      if (editData.productsX) {
        setSelectedXProducts(editData.productsX);
      }
      if (editData.productsY) {
        setSelectedYProducts(editData.productsY);
      }
      ...
      // Set variant pricing if available
      if (editData.originalVariantPrices) {
        setVariantPricing(editData.originalVariantPrices);
      }
  ...
  const fetchProductPricesForEdit = async (editData) => {
    try {
      const allProducts = [...(editData.productsX || []), ...(editData.productsY || [])];
      const variantPrices = [];
  ...
  const handleNext = async () => {
    ...
    if (selectedIndex == tabs.length - 1) {
      if (!selectedXProducts || selectedXProducts.length === 0) {
        shopify.toast.show("Choose products that customer buys.", {
          duration: 4000,
          style: { backgroundColor: "#f44336", color: "#fff" },
        });
        return;
      }
      if (!selectedYProducts || selectedYProducts.length === 0) {
        shopify.toast.show("Choose products that customer gets with a discount or for free.", {
          duration: 4000,
          style: { backgroundColor: "#f44336", color: "#fff" },
        });
        return;
  ...
  const removeXProduct = (productToRemove) => {
    setSelectedXProducts(
      selectedXProducts.filter((product) => product.productId !== productToRemove.productId)
    );
  ...
  const removeYProduct = (productToRemove) => {
    setSelectedYProducts(
      selectedYProducts.filter((product) => product.productId !== productToRemove.productId)
    );
    setPreviewProductsSelectedOptions((prev) => {
      const updated = { ...prev };
      delete updated[productToRemove.productId];
    ...
    setVariantPricing((prev) => {
      ...
      newVariantPrices.forEach((newPrice) => {
        const existingIndex = updatedPricing.findIndex(
          (p) => p.productId === newPrice.productId && p.title === newPrice.title
        );
        if (existingIndex > -1) {
  ...
  const getProductPrice = (productId, productType) => {
    // productType is 'X' or 'Y'
    ...
    if (!variantDetails && variantTitleFromPreview === "") {
      ...
      if (baseProduct && (!baseProduct.optionSelections || baseProduct.optionSelections.length === 0)) {
        variantDetails = variantPricing.find((p) => p.productId === productId && p.title === "Default Title");
                    ...
                    <CardBody>
                      ...
                      >
                        ...
                        <div className="d-flex justify-content-between p-3 mb-1">
                          ...
                          <Button
                            text={
                              <>
                                <Trash style={{ marginRight: "6px" }} /> Clear All
                              </>
                            }
                        ...
                        {selectedXProducts.map((product, idx) => (
                          <CardBody
                            key={`x-${idx}`}
                            className="d-flex align-items-center justify-content-between m-0 p-3 border-top"
                          >
                        ...
                        {selectedXProducts.length === 0 && (
                          <p className="p-3 text-muted">No products selected for "Buy X".</p>
                        )}
                      </div>
                      ...
                      >
                        ...
                        {selectedYProducts.length === 0 && (
                          <p className="p-3 text-muted">No products selected for "Get Y".</p>
                        )}
                      </div>
                    </CardBody>
                  </Card>
                        ...
                        {selectedXProducts.map((product, index) => {
                          ...
                          return (
                            <React.Fragment key={`preview-x-${product.productId || index}`}>
                              ...
                              {/* {(selectedYProducts.length > 0 ||
                                index < selectedXProducts.length - 1) && (
                                <div
                                  style={{
                                    position: "relative",
                                    display: "flex",
                            ...
                            >
                              {discountType === "Percentage"
                                ...
                                : discountType === "Free Gift"
                                  ...
                                  : `YOU GET ${currency}.${inputValue || 0} OFF ON`}
                            </p>
                                                    ...
                                                    <>
                                                      <select
                                                        ...
                                                        style={{
                                                          fontSize: "12px",
                                                          background:
                                                            colorSettings["Secondary Background Color"],
                                                          color: colorSettings["Primary Text Color"],
                                                          padding: "8px 12px",
                                                          borderRadius: "8px",
                                                          border: `1px solid ${colorSettings["Border Color"]}`,
...
export default BuyoneGetoneActions;
