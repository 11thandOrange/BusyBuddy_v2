...
export default function Products({ onClose, setSelectedProducts, selectedProducts = [], setVariantPricing }) {
  ...
  // Function to check if other products should be disabled
  const isProductDisabled = (productId) => {
    const selectedProductIds = Object.keys(selectedVariants).filter((id) =>
      Object.values(selectedVariants[id] || {}).some((isSelected) => isSelected)
    );

    // Allow selection if no products are selected or if the product is already selected
    return selectedProductIds.length > 0 && !selectedProductIds.includes(productId);
  };

  // Function to check if an option should be disabled
  const isOptionDisabled = (productId) => {
    return isProductDisabled(productId);
  };
