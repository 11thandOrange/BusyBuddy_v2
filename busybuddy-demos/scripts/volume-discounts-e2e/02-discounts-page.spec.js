...
// having already created a bundle titled VOLUME_TEST_TITLE (runs first -
// playwright.config.js sets workers: 1 / fullyParallel: false, so file
// order within the directory is preserved).
test('Volume Discount: preview, edit-in-new-tab, and activate from the Discounts list', async ({ page, app }) => {
  await dashboardTile(app, 'Volume Discounts').getByRole('button', { name: /manage/i }).click();
  await gotoTab(app, 'Discounts');
  await demoPause(app);

  const row = discountRowByTitle(app, VOLUME_TEST_TITLE);
  ...
  await expect(app.getByRole('dialog').getByText(VOLUME_TEST_TITLE).first()).toBeVisible();
  await demoPause(app);
  ...
  await expect(editorPopup.locator('input.editable-title')).toHaveValue(VOLUME_TEST_TITLE);
